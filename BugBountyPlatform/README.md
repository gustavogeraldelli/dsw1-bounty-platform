# Bug Bounty Platform (Backend API & MVC)
A documentação do sistema está dividida por etapas de entrega. Clique nos títulos abaixo para expandir os detalhes técnicos e as instruções de execução.
...
Os controladores responsáveis foram isolados no pacote `controller.rest`, separando o tráfego de dados via JSON e requisições multipart dos controladores tradicionais. Para otimizar a comunicação, a aplicação utiliza Records do Java como Data Transfer Objects (DTOs). Além disso, um tratador global de exceções (`@ControllerAdvice`) foi adicionado para interceptar violações de integridade do banco de dados, garantindo respostas HTTP padronizadas (como 409 Conflict).

O filtro do Spring Security foi ajustado para liberar o tráfego e desabilitar a proteção CSRF especificamente para o escopo desta API.

#### Principais Endpoints
* **Programas (`/api/programas`):** Operações completas de CRUD (GET, POST, PUT, DELETE) para o gerenciamento dos escopos de teste.
* **Empresas (`/api/empresas`):** Gerenciamento de empresas (GET, POST, PUT, DELETE), com validação referencial que impede a exclusão de entidades com programas vinculados (retornando erro 409).
* **Pesquisadores (`/api/pesquisadores`):** Gerenciamento de pesquisadores (GET, POST, PUT, DELETE), também protegido contra exclusões de usuários com relatórios ativos.
* **Relatórios (`/api/relatorios`):** 
  * `POST /api/relatorios`: Submissão de novas evidências, recebendo dados e o arquivo PDF via `multipart/form-data`.
  * `PUT /api/relatorios/{id}/avaliar`: Endpoint para a empresa aprovar ou rejeitar uma submissão, consumindo as informações de severidade e recompensa via payload JSON.
...
* O arquivo `README.md` interno com o detalhamento completo de cada rota, mapeamento dos testes e respostas de exceções.
